...
uint16_t hamming_encode_single(uint8_t data) {
    uint8_t D1 = (data >> 3) & 1;
    uint8_t D2 = (data >> 2) & 1;
    uint8_t D3 = (data >> 1) & 1;
    uint8_t D4 = data & 1;
...
uint16_t hamming_encode(uint8_t data) {
    uint16_t fst_part = hamming_encode_single(data);
    uint16_t snd_part = hamming_encode_single(cutter(data));
...
uint8_t hamming_decode_single(uint16_t encoded_data) {
...
uint8_t hamming_decode(uint16_t encoded_data) {
    int8_t fst_part = hamming_decode_single(encoded_data);
    int8_t snd_part = hamming_decode_single(cutter(encoded_data));

    if (fst_part == -1 || snd_part == -1) {
        return -1;
    }
...
int main() {
    ...
    uint8_t data = 4; // 10010110 in binary
    // Encode the data
